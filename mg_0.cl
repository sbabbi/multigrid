...
inline real manhattan(real2 v) { return fabs(v.x)+fabs(v.y);}
...
__kernel void iteration_kernel(global read_only real2 * border,
							...
							real w)
{
	...
	real2 b = border[_X.x + size.x * _X.y];
	...
	else
	{
		...
		dest[_X.x + size.x * _X.y] =  dot (
			(real2)(src[_X.x + convert_int(sign(b.x)) + size.x * _X.y],
				  src[_X.x + size.x *(_X.y + convert_int(sign(b.y)))]), fabs(b) ) - func[_X.x + size.x * _X.y];
...
__kernel void residual_kernel(global read_only real2 * border,
								global write_only real * dest,
								global read_only real * src,
								global read_only real * func,
								int2 size)
{
	...
	{
		val = dot ((real2)(src[_X.x + convert_int(sign(b.x)) + size.x * _X.y],src[_X.x + size.x *(_X.y + convert_int(sign(b.y)))]),
			 fabs(b) ) - manhattan(b)*src[_X.x + size.x *_X.y] -func[_X.x + size.x * _X.y];
	}
...
/**Full-weighting**/
__kernel void reduction_kernel(global read_only real2 * border,
								global write_only real * dest,
								global read_only real *src,
							   int2 size)
{
...
__kernel void residual_correct_kernel(global read_only real2 * border,
									  global write_only real * dest,
										global read_only real *src,
